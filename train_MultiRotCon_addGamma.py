...
def train(args, dataset, valid_dataset):
    ...
    net = models.RotConGE(args.backbone, args.degree)
    net.train()
    net.to(args.device)
    ...
    with open(os.path.join(savepath, "train_log"), 'a') as outfile:
        for epoch in range(1, args.epoches + 1):
            ...
            for i, data in enumerate(dataset):
                ...
                inc_gaze, inc_gaze_dict, inc_rotmat_dict = net(input_inc)
                dec_gaze, dec_gaze_dict, dec_rotmat_dict = net(input_dec)
                ...
                for degree in args.degree:
                    ...
                    inc_rot_matrix = inc_rotmat_dict[f"rot_{degree}_matrix"]
                    inc_rot_matrix = inc_rot_matrix.view(inc_rot_matrix.shape[0], 3, 3)
                    dec_rot_matrix = dec_rotmat_dict[f"rot_{degree}_matrix"]
                    dec_rot_matrix = dec_rot_matrix.view(dec_rot_matrix.shape[0], 3, 3)
                    loss_degree_rotmat = lossfunc(rot_matrix, tg_rot_matrix) + lossfunc(inc_rot_matrix, tg_rot_matrix) + lossfunc(dec_rot_matrix, tg_rot_matrix)
                    ...
                    inc_rot_gaze = inc_gaze_dict[f"out_{degree}_gaze"]
                    inc_inv_gaze = utils.inverse_rotation(inc_rot_matrix, inc_rot_gaze)
                    dec_rot_gaze = dec_gaze_dict[f"out_{degree}_gaze"]
                    dec_inv_gaze = utils.inverse_rotation(dec_rot_matrix, dec_rot_gaze)
                    loss_degree_gaze = lossfunc(inv_gaze, target) + lossfunc(inc_inv_gaze, target) + lossfunc(dec_inv_gaze, target)
...
if __name__ == "__main__":
    ...
    with open(os.path.join(args.root, 'readme.txt'), 'a') as file:
        file.write(f'{current_time}: MultiRotCon_addGamma网络。加入旋转矩阵，直接跨域。利用旋转矩阵还原角度计算损失。' + '\n')
    # 复制文件
